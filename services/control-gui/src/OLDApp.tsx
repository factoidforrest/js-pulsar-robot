...
const App =   () => {
  ...
  // const natWorldProv = Cesium.ImageryLayer.fromProviderAsync(Cesium.IonImageryProvider.fromAssetId(3813),{show:true});

  // const osm = new Cesium.OpenStreetMapImageryProvider({
  //   url : 'https://api.maptiler.com/maps/basic-v2/?key=BT1Fn3eOftBzBesWA9jg',

  // });

  const key = "BT1Fn3eOftBzBesWA9jg";

  const mapTilesLayer = new Cesium.ImageryLayer( new Cesium.UrlTemplateImageryProvider({
  ...
  const viewerRefCallback = async (node: CesiumComponentRef<CesiumViewer>) => {
    ...
    if (node?.cesiumElement) {
      ...
      // scene.imageryLayers.addImageryProvider(osm);
      ...
      scene.imageryLayers.add(mapTilesLayer);
